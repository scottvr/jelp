...
parser = build_parser()
enable_jelp(parser, version=__version__)  # one-line install; parse_args() auto-emits on --jelp*
```

Possible future variants:

```python
enable_jelp(parser, version=__version__, flag="--jelp")
enable_jelp(parser, version=__version__, pretty_flag="--jelp-pretty")
enable_jelp(parser, auto_handle=False)  # use handle_jelp_flag(...) explicitly
```
...
Active checklist status now lives in [docs/task-tracker.md](docs/task-tracker.md).
